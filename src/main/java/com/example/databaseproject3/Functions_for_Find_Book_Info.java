...
public class Functions_for_Find_Book_Info {
    ...
    public static void searchByYearPublished(String yearPub) throws SQLException {
        ...
        Class.forName("com.mysql.cj.jdbc.Driver");
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM home_library.Book WHERE Year_Published = ?")) {
            stmt.setString(1, yearPub);
            ResultSet rs = stmt.executeQuery();
            boolean found = false;
            ...
            if (!found) {
                System.out.println("No books found published in the year: " + yearPub);
            }
        } catch (SQLException e) {
            System.out.println("Unable to connect to the database: " + e.getMessage());
        }
    ...
    public static void searchByPageNum(String numPages) throws SQLException {
        ...
        Class.forName("com.mysql.cj.jdbc.Driver");
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM home_library.Book WHERE Num_of_Pages = ?")) {
            stmt.setString(1, numPages);
            ResultSet rs = stmt.executeQuery();
            boolean found = false;
            ...
            if (!found) {
                System.out.println("No books found with " + numPages + " pages");
            }
        } catch (SQLException e) {
            System.out.println("Unable to connect to the database: " + e.getMessage());
        }
    ...
    public static void searchByGenre(String genre) throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM home_library.book JOIN home_library.genre ON book.Genre = genre.Genre_id WHERE genre.Genre_Name = ?")) {
    ...
    public static void searchByBookType(String type) throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM home_library.Book WHERE Book_Type = ?")) {
